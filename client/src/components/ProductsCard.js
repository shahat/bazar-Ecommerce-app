...
function ProductsCard({ product }) {
  ...
  const handleDetails = (product) => {
    navigator(`product/${product._id}`, { state: product });
  };
  return (
    <div className="group relative">
      <div
        onClick={() => handleDetails(product)}
        className="w-full h-96 cursore-pointer overflow-hidden "
      >
        <img
          src={product.image}
      ...
      <div className=" w-full justify-end border-[1px] px-2 py-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2 relative overflow-hidden w-28 text-sm">
            <div className="flex gap-2 transform group-hover:translate-x-24 transition-transform duration-500">
              ...
              <p className="font-semibold text-yellow-600">${product.price}</p>
            </div>
            <p
              onClick={() =>
                dispatch(
